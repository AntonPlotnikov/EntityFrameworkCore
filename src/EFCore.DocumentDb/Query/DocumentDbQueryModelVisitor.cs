...
namespace Microsoft.EntityFrameworkCore.Query
{
    public class DocumentDbQueryModelVisitor : EntityQueryModelVisitor
    {
        ...
        protected virtual Dictionary<IQuerySource, SelectExpression> QueriesBySource { get; } = new Dictionary<IQuerySource, SelectExpression>();
        ...
        public override void VisitWhereClause(WhereClause whereClause, QueryModel queryModel, int index)
        {
            if (Expression is ShapedQueryExpression)
            {
                ...
                var sqlTranslatingExpressionVisitor = _sqlTranslatingExpressionVisitorFactory.Create(this, selectExpression);

                var sql = sqlTranslatingExpressionVisitor.Visit(whereClause.Predicate);

                if (sql != null)
                {
                    selectExpression.AddFilterCondition(sql);
